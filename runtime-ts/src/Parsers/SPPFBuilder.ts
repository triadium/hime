...
export class SPPFBuilder implements SemanticBody {
	...
	/// </summary>
	get Length(): int { return this.handleNext }
	...
	constructor(tokens: TokenRepository, variables: ROList<GSymbol>, virtuals: ROList<GSymbol>) {
		...
		this.poolHPs = new Pool<SPPFBuilder.HistoryPart>(new SPPFBuilder.HistoryPartFactory(), SPPFBuilder.INIT_HISTORY_SIZE)
		...
		return new Proxy(this, {
			...
			get(target, prop) {
				...
				if (typeof index === 'number' && !isNaN(index)) {
					const reference = self.cacheChildren[self.handleIndices[index]!]!
					const sppfNode = self.sppf.GetNode(reference.NodeId)
					const label = (sppfNode as SPPFNodeNormal).GetVersion(reference.Version).Label
					return self.result.GetSemanticElementForLabel(label)
				}
	...
	private AddToCache(gssLabel: int, action: TreeAction): void {
		...
		if (node.IsReplaceable) {
			...
			for (let i = 0; i != replaceable.ChildrenCount; i++) {
				this.AddNodeToCache(this.sppf.GetNode(replaceable.Children![i]!.NodeId) as SPPFNodeNormal, replaceable.Actions![i]!)
			}
		}
		else {
	...
	Reduce(generation: int, varIndex: int, headAction: TreeAction): int {
		const label = headAction === TreeAction.ReplaceByChildren ? this.ReduceReplaceable(varIndex) : this.ReduceNormal(varIndex, headAction)
		...
		for (let i = 0; i < this.handleNext; ++i) {
			switch (this.handleActions[i]) {
				case TreeAction.Promote:
					...
					ArrayCopy(this.cacheChildren, this.handleIndices[i]! + 1, this.cacheChildren, insertion, promotedVersion.ChildrenCount)

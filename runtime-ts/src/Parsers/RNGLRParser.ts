...
export class RNGLRParser extends BaseLRParser implements IContextProvider {
	...
	constructor(automaton: RNGLRAutomaton, variables: GSymbol[], virtuals: GSymbol[], actions: SemanticAction[], lexer: BaseLexer) {
	...
	GetContextPriority(context: int, onTerminalID: int): int {
		...
		for (const shift of this.shifts) {
			...
			for (let i = 0; i < count; ++i) {
				...
				if (action.Code === LRActionCode.Shift) {
					...
					if (this.parserAutomaton.GetContexts(this.gss.GetRepresentedState(shift.from)).Opens(this.nextToken.TerminalID, context)) {
		...
		for (const vstack of queueVStack) {
			...
			for (let i = 0; i < count; ++i) {
				...
				if (action.Code === LRActionCode.Shift && this.parserAutomaton.GetContexts(state).Opens(onTerminalID, context)) {
	...
	Parse(): ParseResult {
		...
		for (let i = 0; i < count; ++i) {
			...
			if (action.Code === LRActionCode.Shift) {
				...
				this.reductions.Enqueue(new RNGLRParser.Reduction(v0, this.parserAutomaton.GetProduction(action.Data), SPPF.EPSILON))
			}
		}

		while (this.nextToken.TerminalID !== GSymbol.SID_EPSILON) // Wait for ε token
		{
		...
		for (let i = genData.Start; i < genData.Start + genData.Count; ++i) {
			...
			if (this.parserAutomaton.IsAcceptingState(state)) {
				...
				return new ParseResult(new ROList<ParseError>(this.allErrors), this.lexer.Input, this.sppf.GetTree(paths[0]![1]!))
			}
		}

...
export class RNGLRAutomaton {
  ...
  IsAcceptingState(state: int): boolean {
    return (
      this.table[state * this.ncols]!.ActionsCount === 1 &&
      this.actions[this.table[state * this.ncols]!.ActionsIndex]!.Code === LRActionCode.Accept
    )
  }

...
export class FuzzyMatcher {
  ...
  private OnFailure(): TokenMatch {
    this.errors(
      new UnexpectedCharError(
        String.fromCharCode(this.text.GetChar(this.originIndex)),
        this.text.GetPositionAt(this.originIndex),
      ),
    )
  ...
  private ExploreTransitions(
    head: FuzzyMatcherNested.Head,
    stateData: AutomatonState,
    offset: int,
    atEnd: boolean,
  ): void {
    for (let i = 0; i < 256; ++i) {
  ...
  private ExploreInsertionWithDistance(
    head: FuzzyMatcherNested.Head,
    offset: int,
    atEnd: boolean,
    current: char,
    ...
    static FromPreviousWithOffset(previous: Head, state: int, offset: int, distance: int) {
      if (distance < previous.Distance + 1) {
        throw new ArgumentException(
          'The distance for the new head must be at least one more than the distance of the previous head',
          'distance',
        )
      }

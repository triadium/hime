...
export abstract class BaseLexer {
  ...
  RunDFAOnError(originIndex: int): TokenMatch {
    if (this.RecoveryDistance <= 0) {
      this.OnError(
        new UnexpectedCharError(this.text.GetChar(originIndex).toString(), this.text.GetPositionAt(originIndex)),
      )
      ...
      const handler = new FuzzyMatcher(
        this.automaton,
        index,
        this.text,
        this.OnError,
        this.RecoveryDistance,
        originIndex,
      )
      return handler.Run()
    }
